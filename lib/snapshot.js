...
function getSnapshotReport(globalContext, options = {}) {
  ...
  const diffJob = maybeStartSnapshotDiffJob(globalContext, report, options);
  const diffQueue = getSnapshotDiffQueueReport(globalContext);

  report.diffStatus = diffQueue.status;
  report.latestDiffResult = diffQueue.latestResult;
  report.diffQueue = {
    ...diffQueue,
    lastRequest: diffJob
  };
