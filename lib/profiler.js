...
function alertSeverityForTrend(trend) {
  if (trend.consecutiveGrowthCount >= CRITICAL_GROWTH_SAMPLES || trend.totalGrowthBytes >= CRITICAL_GROWTH_BYTES) {
    return "critical";
  }

  if (trend.consecutiveGrowthCount >= WARNING_GROWTH_SAMPLES || trend.totalGrowthBytes >= WARNING_GROWTH_BYTES) {
    return "warning";
  }
...
function buildTrendAlert(trend) {
  const severity = alertSeverityForTrend(trend);
...
function buildExpansionAlert(expander) {
  let severity = null;

  if (expander.deltaBytes >= CRITICAL_GROWTH_BYTES) {
    severity = "critical";
  } else if (expander.deltaBytes >= WARNING_GROWTH_BYTES) {
    severity = "warning";
  }
...
function buildAlerts(records, expanders, limit) {
  const trendAlerts = records
    .map(summarizeTrend)
    .map((trend) => trend && buildTrendAlert(trend))
    .filter(Boolean);
  const expansionAlerts = expanders
    .map(buildExpansionAlert)
    .filter(Boolean);
...
function buildAnalysis(records, limit, options = {}) {
  ...
  const alerts = buildAlerts(records, topExpanders, limit);
