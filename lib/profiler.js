...
function buildTopExpanders(records, limit) {
  ...
  return Array.from(groups.values())
    ...
    .map((group) => {
      ...
      const expansionRatio = receiveBytes && receiveBytes > 0
...
function buildExpansionSuspect(expander) {
  ...
  if (expander.expansionRatio === null) {
...
function buildAnalysis(records, limit) {
  const topExpanders = buildTopExpanders(records, limit);
...
function normalizeFilter(options = {}) {
  ...
  return {
    ...
    minBytes: Number.isFinite(Number(source.minBytes)) ? Number(source.minBytes) : null,
    minDeltaBytes: Number.isFinite(Number(source.minDeltaBytes)) ? Number(source.minDeltaBytes) : null
  };
}
...
function getProfilerReport(globalContext, options = {}) {
  ...
  return {
    ...
    filter,
    records,
    analysis: buildAnalysis(allRecords, limit)
  };
}
