...
function renderDashboardHtml(input = {}, options = {}) {
  ...
  const refreshSeconds = Number.isFinite(Number(options.refreshSeconds))
    ? Math.max(0, Number(options.refreshSeconds))
    : 0;
  ...
  const alertRows = dashboard.tables.alerts.map((alert) =>
    `<tr class="hg-data-row" data-severity="${escapeHtml(alert.severity)}"><td>${escapeHtml(alert.severity)}</td><td>${escapeHtml(alert.kind)}</td><td>${escapeHtml(alert.summary)}</td></tr>`
  ).join("");
  const suspectRows = dashboard.tables.suspects.slice(0, 10).map((suspect) =>
    `<tr class="hg-data-row" data-severity="${escapeHtml(suspect.severity)}"><td>${escapeHtml(suspect.severity)}</td><td>${escapeHtml(suspect.category || suspect.kind)}</td><td>${escapeHtml(suspect.summary)}</td></tr>`
  ).join("");
  const contextTrendRows = dashboard.tables.contextTrends.slice(0, 10).map((trend) =>
    `<tr class="hg-data-row" data-severity="warning"><td>${escapeHtml(trend.scope || "context")}</td><td>${escapeHtml(trend.contextKey || trend.property)}</td><td>${escapeHtml(trend.totalGrowthBytesFormatted)}</td><td>${escapeHtml(trend.consecutiveGrowthCount)}</td></tr>`
  ).join("");
...
${refreshMeta}  <meta name="heap-guardian-refresh-seconds" content="${escapeHtml(refreshSeconds)}">
  ...
  <style>
    ...
    .hg-controls { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; margin: 16px 0; }
    .hg-controls input, .hg-controls select { padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white; }
    ...
    th { color: #4b5563; font-size: 13px; cursor: pointer; user-select: none; }
...
<main>
  ...
  <div class="hg-controls">
    <input id="hg-search" type="search" placeholder="Search tables" aria-label="Search tables">
    <select id="hg-severity" aria-label="Filter severity">
...
<script>
  (function () {
    var search = document.getElementById("hg-search");
    var severity = document.getElementById("hg-severity");

    function applyFilters() {
      var query = (search && search.value ? search.value : "").toLowerCase();
      var selectedSeverity = severity && severity.value ? severity.value : "";

      document.querySelectorAll(".hg-data-row").forEach(function (row) {
        var textMatch = !query || row.textContent.toLowerCase().indexOf(query) !== -1;
        var severityMatch = !selectedSeverity || row.getAttribute("data-severity") === selectedSeverity;
        row.classList.toggle("hg-hidden", !(textMatch && severityMatch));
      });
    }
    ...
    function sortTable(table, columnIndex) {
      ...
      var ascending = table.getAttribute("data-sort-column") !== String(columnIndex)
        || table.getAttribute("data-sort-direction") === "desc";

      rows.sort(function (left, right) {
        var leftText = (left.children[columnIndex] && left.children[columnIndex].textContent || "").trim();
        var rightText = (right.children[columnIndex] && right.children[columnIndex].textContent || "").trim();
        var leftNumber = Number(leftText.replace(/[^0-9.-]/g, ""));
        var rightNumber = Number(rightText.replace(/[^0-9.-]/g, ""));
        var comparison = Number.isFinite(leftNumber) && Number.isFinite(rightNumber)
